...
class Scenario():
    ...
    overwrite: bool
        Overwrite scenario if osrm files already exist
    verbose: bool
...
class ScenarioAPI(AbstractContextManager):
    ...
    and provides an API for querying that server. The low-level API is provided by the
    `python-osrm` module (https://github.com/ustroetz/python-osrm)
    ...
    def __init__(self, scenario, call_opts={}, **kwargs):
        ...
        self.config = osrm.DefaultRequestConfig
    ...
    def _set_config(self, port):
        ...
        self.AccessIsochrone = partial(osrm.AccessIsochrone, url_config=self.config)
