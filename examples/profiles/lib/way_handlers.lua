...
-- SKO BUFFS FLOOD MODEL don't allow routing over footbridges
function WayHandlers.no_footbridges(profile,way,result,data)
  bridge = way:get_value_by_key("bridge")
  highway = way:get_value_by_key("highway")

  if highway == "path" and bridge == "yes" then
    return false
  end
end

-- SKO BUFFS FLOOD MODEL don't allow routing over flood prone bridges
function WayHandlers.no_flood_prone(profile,way,result,data)
  bridge = way:get_value_by_key("bridge")
  flood_prone = way:get_value_by_key("flood_prone")

  if bridge == "yes" and flood_prone == "yes" then
    return false
  end
end
